...
#@Integer(label="Site channel", value=2) siteChannel
...
def processFile(file) {
	...
	impC1 = new Duplicator().run(imp, 1, 1, 1, imp.nSlices, 1, 1)
	IJ.run(impC1, "Subtract Background...", "rolling=" + ballSize1 + " stack")
	impC2 = new Duplicator().run(imp, 2, 2, 1, imp.nSlices, 1, 1)
	...
	if(ok == 1) {
		...
		if (siteChannel == 1) {
			graph = constructGraph(spotsC1, linkingDist)
		} else if (siteChannel == 2){
